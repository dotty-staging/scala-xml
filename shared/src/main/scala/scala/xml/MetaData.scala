...
object MetaData {
  ...
  def getUniversalKey(attrib: MetaData, scope: NamespaceBinding) = attrib match {
    case prefixed: PrefixedAttribute     => String.valueOf(scope.getURI(prefixed.pre)) + prefixed.key
...
abstract class MetaData
   ...
   */
  final def apply(namespace_uri: String | Null, owner: Node, key: String): Seq[Node] | Null =
   ...
   * @return value as Seq[Node] if key is found, null otherwise
   */
  def apply(namespace_uri: String | Null, scp: NamespaceBinding, k: String): Seq[Node] | Null
